...
void	execute_echo(char *inpt)
{
	...
	while (inpt[i] == ' ' || inpt[i] == '\t')
		i++;
	//check for -n flag
	//(inpt[i + 2] == ' ' || ...) ensures that the
		-n flag is a standalone flag and not part of the text to be echoed
...
void	execute_cd(char *inpt)
{
	//Relative Path: A path that is relative to the current directory. For example,
		cd folder changes to a subdirectory folder within the current directory.
	//Absolute Path: A path that starts from the root directory. For example, cd
		/home/user/folder changes to the specified directory starting from the root.
	//init necessary var
...
void	execute_unset(char *inpt)
{
	...
	while (inpt[i] && inpt[i] != ' ' && inpt[i] != '\t' && j < sizeof(varname)
		- 1)
		varname[j++] = inpt[i++];
	varname[j] = '\0';
...
void	execute_export(char *inpt)
{
	...
	new_environ[env_size] = malloc(ft_strlen(varname) + ft_strlen(value)
			+ 2);//+2 for = and \0
