...
char	**pipe_tokenizer(char *command, int *num_commands)
{
	...
	while (token != NULL)
	{
		commands[i] = strdup(token);
			// Duplicate the token and store it in the commands array
		if (!commands[i])
		{
			perror("strdup");
			while (i > 0)
...
void	execute_pipeline(char **commands, int num_commands)
{
	...
	int fd_in;    
		// stores the file descriptor for the input of the next command
	int i;
	...
	while (i < num_commands)
	{
		...
		{
			dup2(fd_in, 0);
				// redirects the input to the previous command's output
			if (i < num_commands - 1)
				dup2(pipefd[1], 1);
					// changes the output to the end of the pipe only if it's not the last command
			close(pipefd[0]);
			close(pipefd[1]);
